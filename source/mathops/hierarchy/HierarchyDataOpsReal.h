//
// File:	$URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-4-4/source/mathops/hierarchy/HierarchyDataOpsReal.h $
...
/**
 * Class HierarchyDataOpsReal<DIM> defines the interface to a collection 
 ...
 * HierarchyDataOpsComplex<DIM> and HierarchyDataOpsInteger<DIM>, 
 * respectively.
 */
...
public:
    ...
    */
   virtual 
   void setPatchHierarchy(tbox::Pointer< hier::PatchHierarchy<DIM> > hierarchy) = 0;
    ...
    */
   virtual void resetLevels(const int coarsest_level, 
    ...
    */
   virtual
   const tbox::Pointer< hier::PatchHierarchy<DIM> > getPatchHierarchy() const = 0;

   /**
    * Copy source data to destination data.
    */
   virtual void copyData(const int dst_id,
                         const int src_id,
    ...
    */
   virtual void setToScalar(const int data_id,
                            const TYPE& alpha,
                            const bool interior_only = true) const = 0;
    ...
    */
   virtual void scale(const int dst_id,
                      const TYPE& alpha,
                      const int src_id,
                      const bool interior_only = true) const = 0;
    ...
    */
   virtual void addScalar(const int dst_id,
                          const int src_id,
                          const TYPE& alpha,
                          const bool interior_only = true) const = 0;
    ...
    */
   virtual void add(const int dst_id,
                    const int src1_id,
                    const int src2_id,
                    const bool interior_only = true) const = 0;
    ...
    */
   virtual void subtract(const int dst_id,
                         const int src1_id,
                         const int src2_id,
                         const bool interior_only = true) const = 0;
    ...
    */
   virtual void multiply(const int dst_id,
                         const int src1_id,
                         const int src2_id,
                         const bool interior_only = true) const = 0;
    ...
    * and set destination data component to result.
    */
   virtual void divide(const int dst_id,
                       const int src1_id,
                       const int src2_id,
                       const bool interior_only = true) const = 0;
    ...
    */
   virtual void reciprocal(const int dst_id,
                           const int src_id,
                           const bool interior_only = true) const = 0;
    ...
    */
   virtual void linearSum(const int dst_id,
                          const TYPE& alpha,
                          const int src1_id,
                          const TYPE& beta,
                          const int src2_id,
    ...
    */
   virtual void axpy(const int dst_id,
                     const TYPE& alpha,
                     const int src1_id,
                     const int src2_id,
                     const bool interior_only = true) const = 0;
    ...
    * respectively.  Here \f$\alpha\f$ is a scalar.
    */
   virtual void axmy(const int dst_id,
                     const TYPE& alpha,
                     const int src1_id,
                     const int src2_id,
                     const bool interior_only = true) const = 0;
    ...
    */
   virtual void abs(const int dst_id,
                    const int src_id,
                    const bool interior_only = true) const = 0;
    ...
    */
   virtual void setRandomValues(const int data_id,
                                const TYPE& width,
                                const TYPE& low,
                                const bool interior_only = true) const = 0;
    ...
    * over all patches.
    */
   virtual int numberOfEntries(const int data_id,
                               const bool interior_only = true) const = 0;
    ...
    * hierarchy levels.
    */
   virtual double sumControlVolumes(const int data_id,
    ...
    */
   virtual double L1Norm(const int data_id,
                         const int vol_id = -1,
    ...
    * performed (thus each process will get only local results).
    */
   virtual double L2Norm(const int data_id,
                         const int vol_id = -1,
			 bool local_only=false) const = 0;
    ...
    */
   virtual double weightedL2Norm(const int data_id,
                                 const int weight_id,
    ...
    */
   virtual double RMSNorm(const int data_id,
                          const int vol_id = -1) const = 0;
    ...
    */
   virtual double weightedRMSNorm(const int data_id,
                                  const int weight_id,
                                  const int vol_id = -1) const = 0;
    ...
    * performed (thus each process will get only local results).
    */
   virtual double maxNorm(const int data_id,
                          const int vol_id = -1,
			  bool local_only=false) const = 0;
    ...
    * performed (thus each process will get only local results).
    */
   virtual TYPE dot(const int data1_id,
                    const int data2_id, 
                    const int vol_id = -1,
		    bool local_only=false) const = 0;
    ...
    */
   virtual int computeConstrProdPos(const int data1_id,
                                    const int data2_id,
                                    const int vol_id = -1) const = 0;
    ...
    * are considered.
    */
   virtual void compareToScalar(const int dst_id,
                        const int src_id,
                        const TYPE& alpha,
                        const int vol_id = -1) const = 0;
    ...
    */
   virtual int testReciprocal(const int dst_id,
                              const int src_id,
                              const int vol_id = -1) const = 0;
    ...
    */
   virtual TYPE maxPointwiseDivide(const int numer_id,
				   const int denom_id,
				   bool local_only=false) const = 0;
    ...
    */
   virtual TYPE minPointwiseDivide(const int numer_id,
				   const int denom_id,
				   bool local_only=false) const = 0;
